...
/**
 * Deploys a contract named "Treasury" using the deployer account
 *
...
const deployTreasury: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  */
  ...
  console.log("🔑 Deployer has DEFAULT_ADMIN_ROLE:", await treasury.hasRole(await treasury.DEFAULT_ADMIN_ROLE(), deployer));
  console.log("🔑 Deployer has TREASURY_ADMIN_ROLE:", await treasury.hasRole(await treasury.TREASURY_ADMIN_ROLE(), deployer));
};

export default deployTreasury;

// Tags are useful if you have multiple deploy files and only want to run one of them.
// e.g. yarn deploy --tags Treasury
